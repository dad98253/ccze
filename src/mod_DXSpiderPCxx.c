 ...
 * mod_DXSpiderPCxx.c -- DXSpiderPCxx-related colorizers for CCZE
...
ccze_DXSpiderPCxx_process (const char *str, int *offsets, int match)
{
  ...
  if ( num[0] == '9' ) {
  	ccze_addstr (CCZE_COLOR_STATIC_BOLD_RED, "PC");
  	ccze_addstr (CCZE_COLOR_STATIC_BOLD_RED, num);  
  } else {
  	ccze_addstr (CCZE_COLOR_STATIC_RED, "PC");
  	ccze_addstr (CCZE_COLOR_STATIC_RED, num);
  }
  msg2 = xstrdup (msg);
  word = xstrdup (ccze_strbrk (msg2, '^'));
  do {
      if (word != NULL) {
      	  ccze_addstr (CCZE_COLOR_STATIC_BLUE, "^");
		  if ( isNumber_dx(word) ) {
		  ccze_addstr (CCZE_COLOR_STATIC_MAGENTA, word);
		  } else {
		  ccze_addstr (CCZE_COLOR_STATIC_BOLD_GREEN, word);
		  }
		  free(word);
		  word = NULL;
	  } else {
  ...
  free (msg2);
  ccze_addstr (CCZE_COLOR_STATIC_BLUE, "^H");
  ccze_addstr (CCZE_COLOR_STATIC_BOLD_BLUE, hops);
  ccze_addstr (CCZE_COLOR_STATIC_BLUE, "^");
  if ( send != NULL ) {
  	  toret = xstrdup (send);
  	  return(toret);
